...
4. LLM-слой (agents.DialogManager, Gemini): распознанный текст команды
   больше не уходит в UI как есть - сначала идёт в LLM с function calling
   (см. agents/tools.py), и в UI отправляется уже осмысленный ответ.
   Требует переменную окружения GEMINI_API_KEY (см. README.md). Если ключ
   не задан, сервер не падает - просто работает в режиме чистого STT-эхо,
   как раньше.
"""
